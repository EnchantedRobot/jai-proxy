...
| `vendor/fontawesome/css/`, `vendor/fontawesome/webfonts/` | Icon font upstream pulled from SillyTavern's server root (`/webfonts`, `/css`). Nothing serves that root now, and the icons are glyphs with no system equivalent, so they stay vendored. The body text face does **not**: `vendor/notosans/` (1.6 MB of woff2) was dropped and `library.css` now uses a system font stack ending in `'Noto Sans'`, which most Linux desktops have installed anyway. |
| `img/` | The two placeholder images upstream loaded from SillyTavern's server root: `ai4.png` (the fallback avatar every browse/grid `onerror` falls back to, 16 call sites) and `No-Image-Placeholder.svg` (the gallery one). Copied verbatim from `SillyTavern/public/img/`. Nothing serves that root now, so without them a grid of cards with unreachable remote avatars turns into a flood of 404s — and an `onerror` whose fallback also 404s re-fires. |

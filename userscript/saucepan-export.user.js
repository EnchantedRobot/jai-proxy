...
(function () {
  ...
  // The real character name for the export-name prompt: companion.name (not
  // display_name, which is the card-title blurb).
  function companionName(export_) {
    const c = (export_.companion && export_.companion.companion) || export_.companion || {};
    return (c.name || "").trim();
  }

  // ---------------------------------------------------------------------------
  // UI: one floating button. Fetch → confirm name → build → report.
  // ---------------------------------------------------------------------------
  async function run(setStatus) {
    ...
    const suggested = companionName(export_) || "";
    const chosen = window.prompt("Card name (server maps the rest):", suggested);
    if (chosen === null) {
      setStatus("Cancelled");
      return;
    }
    const output_name = chosen.trim();

    setStatus("Building on server…");
    const payload = { character: export_ };
    if (output_name && output_name !== suggested) payload.output_name = output_name;

    const res = await buildOnServer(payload);
    if (res.ok) {
